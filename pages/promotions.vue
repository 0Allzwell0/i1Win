		...
		<ul class="promotions-list">
			<li class="promotions-item" v-for="(item, index) in promotionsList" :key="`promotion-${index}`">
				...
				<div class="promotions-btn-wrapper">
					<!-- Detail -->
					<button class="promotions-btn detail-btn" data-toggle="modal" :data-target="`#detail_${index}`" type="button">
						{{ $t('promotions.detail') }}
					</button>

					<!-- Register => 尚未登入時顯示 -->
					<nuxt-link class="promotions-btn right-btn" :to="$i18n.path('register')" v-if="!isLogined">
						{{ $t('common.register') }}
					</nuxt-link>
					<!-- Deposit => 已登入後才顯示 -->
					<nuxt-link class="promotions-btn right-btn" :to="$i18n.path('member/deposit')" v-if="isLogined">
						{{ $t('promotions.deposit') }}
					</nuxt-link>
				</div>
